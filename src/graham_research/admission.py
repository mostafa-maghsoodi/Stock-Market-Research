...
RESEARCHER_DECISIONS = (
    ...
    },
    {
        ...
        "why_required": "date, ticker, and venue can identify one Databento instrument but do not themselves authorize stable internal issuer/security/listing identities or the separate run-specific crosswalk evidence identity",
        "governing_clause": "Screen v2 Section 12 items 3, 7, and 14",
        "allowed_domain": "closed stable issuer/security/listing identities from lifecycle-stable CIK, share-class FIGI, MIC, and listing boundary material, plus a separate content-bound observation identity for date-effective Massive/Databento/SEC evidence",
        "options": "approve the stable/provenance separation or another exact scheme that keeps decision dates, acquisition timestamps, manifests, run IDs, and evidence hashes out of stable IDs and never treats ticker alone as identity",
        "tradeoffs": "stable IDs support multi-date lineage; separate evidence identities preserve exact reproducibility and may legitimately change between runs",
        "framing": "require one and only one date-effective venue match, stable IDs across equivalent observations, and fail closed on missing lifecycle material or ambiguous joins",
        "successor_governance_if_changed": True,
        "authority_path": "NEW_AUTHORITY_REQUIRED",
        "timestamp_or_attestation_required": True,
        "downstream_field": "stable issuer_id/security_id/listing_id and separate CrosswalkEvidenceIdentity",
        "required_before_first_run": True,
    },
)
